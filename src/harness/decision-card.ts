...
const DECIDE_TOOL_SCHEMA = {
  ...
  parameters: {
    ...
    properties: {
      ...
      options: { type: "array", items: { type: "string" }, description: "2-4 个候选方向，每个是具体可行的剧情走向" },
      allow_free_input: { type: "boolean", description: "是否允许用户自由输入自定义走向，默认 true" },
    },
...
export function parseDecisionCard(argsJson: string): DecisionCard | null {
  try {
    ...
    if (!Array.isArray(a.options) || a.options.length === 0 || a.options.some((o) => typeof o !== "string")) return null;
    return {
      question: a.question,
...
/** 决策留痕：state/decisions.jsonl，日后可回看每个岔路口 */
export function recordDecision(stateDir: string, card: DecisionCard, choice: string): void {
  mkdirSync(stateDir, { recursive: true });
  const record: DecisionRecord = { ...card, at: new Date().toISOString(), choice };
  appendFileSync(resolve(stateDir, "decisions.jsonl"), JSON.stringify(record) + "\n", "utf8");
}
