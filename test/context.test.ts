...
function makeMockClient(scriptedSummaries: string[]): LlmClient & { calls: number } {
  let calls = 0;
  const fake = {
    calls: 0,
    async chat(): Promise<ChatResult> {
      calls++;
      fake.calls = calls;
      const content = scriptedSummaries[Math.min(calls - 1, scriptedSummaries.length - 1)] ?? "";
...
function turn(user: string, narrative: string): { userInput: string; added: ChatMessage[] } {
  return { userInput: user, added: [{ role: "assistant", content: narrative }] };
}
...
test("上下文：组装顺序 = system → 前情提要 → 窗口 → 用户输入", async () => {
  ...
  try {
    const ctx = new ContextManager(makeMockClient(["旧摘要"]), makeConfig(dir, 100_000));
    await ctx.endTurn({ systemText: "SYS", ...turn("第一回合用户", "第一回合正文") });
...
test("上下文：预算超限时压缩最旧回合，原文进归档", async () => {
  ...
  try {
    ...
    // 归档里有原文
    const archive = readFileSync(join(dir, "archive.jsonl"), "utf8");
    assert.ok(archive.includes("深夜加班被王总骂"));
  } finally {
    rmSync(dir, { recursive: true, force: true });
  }
});
...
test("上下文：摘要异常（过短）时保留旧摘要降级", async () => {
  const dir = mkdtempSync(join(tmpdir(), "rph-ctx-"));
  try {
    const client = makeMockClient(["好的完整摘要第一版，包含王总与便利店老板娘的信息", "短"]);
    const ctx = new ContextManager(client, makeConfig(dir, 120));
    await ctx.endTurn({ systemText: "SYS", ...turn("剧情一", longText("正文一内容，需要被压缩成摘要以便后续合并")) });
    const first = ctx.summaryText;
    await ctx.endTurn({ systemText: "SYS", ...turn("剧情二", longText("正文二内容，同样要被压缩")) });
...
test("上下文：archiveSearch 可检索被压缩的原文", async () => {
  const dir = mkdtempSync(join(tmpdir(), "rph-ctx-"));
  try {
    const ctx = new ContextManager(makeMockClient(["摘要摘要"]), makeConfig(dir, 200));
    await ctx.endTurn({ systemText: "SYS", ...turn("我在便利店见到了妖狐老板娘", longText("正文……")) });
