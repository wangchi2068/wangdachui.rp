...
| 模块 | 说明 |
|---|---|
| **主线导演层** | 三幕六阶段主线大纲（觉醒→追查→真相）：关键词+回合门槛规则推进，每轮注入【主线】段与一次性事件钩子，剧情有骨架又不失自由；主线进度随世界线快照一起回档 |
| **群像叙事** | 9 人群像卡（剑灵玄一/老板娘苏涟漪/王总/妖狐小九/城隍老太太/鬼市老瞎子/猫妖阿贵/表妹圆圆…），每个角色有世界书条目与主线关联；侧边栏人物册实时登记 |
| **上下文工程** | 模型每轮只看到三层：system（人设+世界书+账本+主线）→ 前情提要（早期剧情的旁侧模型压缩产物）→ 滑动窗口（近期原文）。超预算时最旧原文回合被压成"剧情化摘要"，原文归档到 `archive.jsonl` 可检索 |
...
| **角色卡/世界书** | SillyTavern v1/v2 JSON **与 PNG 内嵌卡**（手写 PNG chunk 解析）、世界书常驻 + 关键词激活 |
